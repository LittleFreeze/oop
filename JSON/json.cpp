...
Value* parse_value(const char*& first, const char* last,std::list <Value*>* mylist);
...
/// Parse an array.
Array*
parse_array(const char*& first, const char* last,std::list <Value*>* mylist)
{
  ...
  while (true) {
    ...
    Value* v = parse_value(first, last, mylist);
    arr->push_back(v);
...
/// Parse an object.
Object*
parse_object(const char*& first, const char* last,std::list <Value*>* mylist)
{
  ...
  while (true) {
    ...
    if(*k == "\"author\"")
    {
      author* a = dynamic_cast<author*>(v);
      mylist->push_back(a);
      std::cout << "Author: " << *v << std::endl;
    }

    if(*k == "\"title\"")
    {
      title* t = dynamic_cast<title*>(v);
      mylist->push_back(t);
...
Value*
parse_value(const char*& first, const char* last,std::list <Value*>* mylist)
{
  if (is_eof(first, last))
    return nullptr;
  ...
  switch (*first) {
    ...
    case '"':
      return parse_string(first, last);
    case '[':
      return parse_array(first, last,mylist);
    case '{':
      return parse_object(first, last,mylist);
...
Value* 
parse(const std::string& str,std::list <Value*>* mylist)
{
  const char* first = str.c_str();
  const char* last = first + str.size();
  return parse_value(first, last,mylist);
}
