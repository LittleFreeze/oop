...
void Deck::addBottom(Card c) //Function to add card to bottom of the deck
{
    ...
    if(c.getKind() == Joker)
    {
        newCard = new JokerCard(c.getColor());
    }
    else
    {
        StandardCard tempCard = dynamic_cast<StandardCard&>(c);
        newCard = new StandardCard(tempCard.getRank(),tempCard.getSuit());
    }
...
Card Deck::pullTop() //Function to pull off the top card of the deck
{
    if(!TopCard)
    {
...
Card* Deck::removeCard(int i, Card*& c) //Function to remove a card at passed position from a deck
{
...
void Deck::ShuffleDeck() //Function to shuffle deck
{
    ...
    else
    {
        ...
        for(int i = count; i > 0; i--)
        {
            ...
            {
                StandardCard* tempCard = dynamic_cast<StandardCard*>(c);
                pushTop(tempCard->getRank(),tempCard->getSuit()); 
            }
            else
            {
                pushJTop(c->getColor());
...
Card* Deck::readTop() //Function to return pointer to top card
{
    return TopCard;
}
